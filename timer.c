...
static volatile uint8_t reset_rs232_time;

void softtimer_clr_rs232_reset(void) {
	reset_rs232_time = 0;
}
...
ISR(TIMER2_OVF_vect) {
	...
	reset_rs232_time++;
	/* 50 ms without reception -> clear RX buffer */
	if (reset_rs232_time >= 50) {
		reset_rs232_time = 0;
		softtimer_ovf_rs232_reset();
	}
}

void timer_init(void) {
	TCNT2 = 0x83;					/* 1 ms */
	TCCR2 = _BV(CS21) | _BV(CS20);	/* CK / 32 = 125 kHz */
	TIMSK = _BV(TOIE2);	
}
